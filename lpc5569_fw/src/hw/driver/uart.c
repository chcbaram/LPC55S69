...
bool uartOpen(uint8_t channel, uint32_t baud)
{
  ...
  switch(channel)
  {
    case _DEF_UART1:
      ...
      p_uart->handle =  USART0;

      qbufferCreate(&p_uart->qbuffer_rx, p_uart->rx_buf, UART_RX_BUF_LENGTH);

      p_uart->is_open  = true;

      USART_SetBaudRate(p_uart->handle, baud, FLEXCOMM0_CLOCK_SOURCE);
...
int32_t uartWrite(uint8_t channel, uint8_t *p_data, uint32_t length)
{
  ...
  {
    USART_WriteBlocking(p_uart->handle, (const uint8_t *)p_data, length);
    ret = length;
  }
...
void FLEXCOMM0_FLEXCOMM_IRQHANDLER(void)
{
  ...
  if ((kUSART_RxFifoNotEmptyFlag | kUSART_RxError) & USART_GetStatusFlags(p_uart->handle))
  {
    data = USART_ReadByte(p_uart->handle);
